...
[shell]
# Program the shell tool and user `!` commands run with: `bash` (the
# default on Linux and macOS), `pwsh` (the default on Windows), `fish`,
# `zsh`, or an absolute path. Known Windows shells get their own command
