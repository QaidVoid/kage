...
//! event callback bridges into the loop's
//! [`CancelFlag`](kage_core::CancelFlag), so the run unwinds at the
//! loop's next safe point and the session file closes cleanly. A second Ctrl-C only sets the same flag again: while the
