...
/// token budget (using the model catalog's per-model table when
/// present, falling back to provider defaults). `OpenAI`'s Chat
/// Completions and Responses APIs translate to the `reasoning_effort`
/// enum string.
///
...
impl ThinkingLevel {
    ...
    /// (Anthropic, Gemini) when the model catalog has no per-model
    /// entry. Returns `None` for [`Self::Off`].
    #[must_use]
