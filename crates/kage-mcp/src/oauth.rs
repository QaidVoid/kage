...
pub(crate) mod tests {
    ...
    /// One request the fake server saw, header names lowercased.
    #[derive(Clone, Debug)]
    ...
    impl FakeServer {
        ...
        pub(crate) fn find(&self, method: &str, path: &str) -> Option<Seen> {
            let seen = self.seen.lock().unwrap();
            seen.iter()
    ...
    pub(crate) fn serve(
        handler: impl Fn(&Seen, &str) -> Reply + Send + Sync + 'static,
    ) -> FakeServer {
        ...
        std::thread::spawn(move || {
            for stream in listener.incoming() {
                ...
                let (handler, log, origin) =
                    (Arc::clone(&handler), Arc::clone(&log), origin.clone());
                std::thread::spawn(move || {
    ...
    /// refresh yields, which is stored from then on. Counts the
    /// refreshes asked for.
