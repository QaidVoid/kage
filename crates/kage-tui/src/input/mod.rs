...
//!
//! This is the editor grammar: vim motions, operators, counts and
//! registers, the readline edits, Enter and Esc, and the modeless
//! empty-prompt prefixes. Command-level keys that only emit an action
//! (`:`, `/`, `j`/`k` in the conversation pane, the pickers) are
//! keymaps the host resolves before a key reaches this state machine.
